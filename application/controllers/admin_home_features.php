...
class Admin_home_features extends CI_Controller {
	
	function __construct(){
		parent::__construct();
		$this->load->model('admin_home_features_model');
	...
	public function index() {
			...
			$data_head['add_button'] = site_url().'admin_home_features/form/';
			...
			$list =  $this->admin_home_features_model->list_data();
			
			$this->load->view('admin_layout/header', array( 'data_head' => $data_head, 'data_user' => $data_user));
			$this->load->view('admin_home_features/list', array('data_head' => $data_head, 'list' => $list));
	...
	public function form($id = 0) {
			...
			$data_head['action'] = site_url().'admin_home_features/form_action/'.$id;
			$data_head['close_button'] = site_url().'/admin_home_features/';
			...
			$data['row_id'] = "";
			if($id){
				$result = $this->admin_home_features_model->read_id($id);
				if($result){
					$data  = $result;
			...
			$this->load->view('admin_layout/header', array( 'data_head' => $data_head, 'data_user' => $data_user));
			$this->load->view('admin_home_features/form', array('data_head' => $data_head, 'data' => $data));
			$this->load->view('admin_layout/footer'); 
	...
	public function form_action($id = 0) {
		...
		if($_FILES['i_img']['name']){
			...
			if($id){
				$get_img = $this->admin_home_features_model->get_img("home_features", "feature_img", "feature_id = '$id'");
			
				$oldfile   = "assets/images/home_features/".$get_img;
		...
		if($id){
			$this->admin_home_features_model->update($data, $id);
		}else{
			$data['feature_date'] = date("Y-m-d H:m:s");
			$this->admin_home_features_model->create($data);
		}
		
		redirect('admin_home_features/?did=2');
	...
	public function upload_img($img){
		$new_name = time()."_".$_FILES[$img]['name'];
			...
			$configUpload['upload_path']    = './assets/images/home_features/';                 #the folder placed in the root of project
